...
describe('Fluxo de Login', () => {
  it('Cenário 1: Deverá ter sucesso ao fazer login e gravará o token no local storage.', () => {
    ...
    // Passo 2: Preencher o formulário com dados válidos
    ...
    // Passo 4: Verificar se a notificação de sucesso do Quasar aparece com a mensagem correta
    cy.get('.q-notification')
      .should('be.visible')
      .and('contain', 'Login realizado com sucesso');

    // Passo 5: Verificar se o token foi salvo no local storage
    cy.window().then((win) => {
  ...
  it('Cenário 2: Deverá falhar ao realizar login e não gravará o token no local storage.', () => {
    ...
    // Passo 2: Preencher o formulário com dados inválidos
    ...
    // Passo 4: Verificar se a notificação de erro do Quasar aparece com a mensagem correta
    cy.get('.q-notification')
      .should('be.visible')
      .and('contain', 'Credenciais inválidas');

    // Passo 5: Verificar se o token não foi salvo no local storage
    cy.window().then((win) => {
      const token = win.localStorage.getItem('token');
